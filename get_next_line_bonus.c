...
/*   Updated: 2023/01/21 18:11:48 by bluzi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*make_line(char **remainder, int nl_pos, int rem_len)
{
	...
	{
		line = ft_calloc(rem_len + 1, sizeof(char));
	...
	else
	{
		...
		buf = ft_calloc(rem_len - nl_pos + 1, sizeof(char));
...
char	*get_next_line(int fd)
{
	...
	if (!remainder[fd])
	{
		remainder[fd] = ft_calloc(BUFFER_SIZE + 1, sizeof(char));
		if (!remainder[fd])
			return (NULL);
	}	
	else
		ft_strealloc(&remainder[fd], size + BUFFER_SIZE + 1);
	return (read_line(&remainder[fd], fd, size));
}
